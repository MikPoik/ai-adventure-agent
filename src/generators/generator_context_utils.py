...
_CHAT_IMAGE_GENERATOR_KEY = "chat-image-generator"
...
def get_chat_image_generator(
    context: AgentContext,
) -> Optional[ImageGenerator]:
    #logging.warning("get_chat_background_image_generator")
    generator = context.metadata.get(_CHAT_IMAGE_GENERATOR_KEY)
    if not generator:
        # Lazily create
        server_settings = get_server_settings(context)
        theme = get_theme(server_settings.quest_background_theme, context)
        generator = get_image_generator(theme)
        context.metadata[_CHAT_IMAGE_GENERATOR_KEY] = generator
    return generator
