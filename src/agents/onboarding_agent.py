...
class OnboardingAgent(InterruptiblePythonAgent):
    ...
    def run(self, context: AgentContext) -> Action:  # noqa: C901
        ...
        if not game_state.chat_history_for_onboarding_complete:
            ...
            onboarding_message = f"""You are a game master for an online quest game. In this game, players go on multiple quests in order to achieve an overall goal. You will tell the story of each quest. By completing quests, players build progress towards an overall goal. Quests take place in a specific location, involve obstacles that must be overcome throughout the quest, and end with the player having either found an item that will help them in subsequent quests, or having achieved their ultimate goal. 

## Game attributes
A player has requested a new game with the following attributes:

Narrative tone: {server_settings.narrative_tone}

Background on the world of the story:
...
Goal
{game_state.player.name}'s overall goal is to: {server_settings.adventure_goal}. Each quest that {game_state.player.name} goes on MUST further them towards this overall goal.


### Note
Please always return content in the narrative voice: {server_settings.narrative_voice}
"""
